...
class QasmSimulatorPy(BaseBackend):
    ...
    def __init__(self, configuration=None, provider=None):
        super().__init__(
            configuration=(
                configuration
                or QasmBackendConfiguration.from_dict(self.DEFAULT_CONFIGURATION)
            ),
            provider=provider,
        )
    ...
    def _add_sample_measure(self, measure_params, num_samples):
        ...
        samples = self._local_random.choice(
            range(2 ** num_measured), num_samples, p=probabilities
        )
        ...
        for sample in samples:
            ...
            for count, (qubit, cmembit) in enumerate(sorted(measure_params)):
                ...
                membit = 1 << cmembit
                classical_memory = (classical_memory & (~membit)) | (
                    qubit_outcome << cmembit
                )
