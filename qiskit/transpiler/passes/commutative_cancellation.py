            ...
            for com_set_idx, com_set in enumerate(wire_commutation_set):
                ...
                for node in com_set:
                    ...
                    if num_qargs == 1 and node.name in q_gate_list:
                        cancellation_sets[(node.name, wire_name, com_set_idx)].append(
                            node
                        )
                    if num_qargs == 1 and node.name in ["z", "u1", "rz", "t", "s"]:
                        cancellation_sets[
                            ("z_rotation", wire_name, com_set_idx)
                        ].append(node)
                    if num_qargs == 1 and node.name in ["rx", "x"]:
                        cancellation_sets[
                            ("x_rotation", wire_name, com_set_idx)
                        ].append(node)
                    ...
                    # it should be dealt with by optimized1qgate pass
                        ...
                        second_op_name = "{0}[{1}]".format(
                            str(node.qargs[1].register.name), str(node.qargs[1].index)
                        )
                        q2_key = (
                            node.name,
                            wire_name,
