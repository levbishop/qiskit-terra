...
def trace_to_fid(trace):
    """Average gate fidelity is Fbar = (d + |Tr (U_target.U^dag)|^2) / d(d+1)
...
class TwoQubitBasisDecomposer():
    ...
    def traces(self, target):
        """Give the expected traces |Tr(U.U_target^dag)| for different number of basis gates"""
    ...
    def decomp0(target):
        ...
        Result Ur has trace:
        |Tr(Ur.U_target^dag)| = 4|(cos(x)cos(y)cos(z)+ j sin(x)sin(y)sin(z)|,
    ...
    def decomp1(self, target):
        ...
        Result Ur has trace:
        |Tr(Ur.U_target^dag)| = 4|cos(x-a)cos(y-b)cos(z-c) + j sin(x-a)sin(y-b)sin(z-c)|,
