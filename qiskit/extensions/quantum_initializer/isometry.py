...
class Isometry(Instruction):
    ...
    def _gates_to_uncompute(self):
        ...
        remaining_isometry = self.params[0].astype(
            complex
        )  # note: "astype" does copy the isometry
        diag = []
        ...
        if len(diag) > 1 and not _diag_is_identity_up_to_global_phase(diag):
            circuit.diagGate(np.conj(diag).tolist(), q_input)
        return circuit
    ...
    def _disentangle(self, circuit, q, diag, remaining_isometry, column_index, s):
        ...
        if not _ucg_is_identity_up_to_global_phase(single_qubit_gates):
            ...
            single_qubit_gates = _merge_UCG_and_diag(
    ...
    def _find_squs_for_disentangling(self, v, k, s):
        ...
        squs = [
            _reverse_qubit_state(
                [
                    v[2 * l * 2 ** s + _b(k, s), k_prime],
                    v[(2 * l + 1) * 2 ** s + _b(k, s), k_prime],
                ],
                _k_s(k, s),
            )
    ...
    def _append_ucg_up_to_diagonal(
        self, circ, q, single_qubit_gates, control_labels, target_label
    ):
        (
            q_input,
            q_ancillas_for_output,
    ...
    def _append_mcg_up_to_diagonal(self, circ, q, gate, control_labels, target_label):
        ...
        # as dirty ancillas
        ancilla_dirty_labels = [
            i for i in range(n) if i not in control_labels + [target_label]
        ]
        ancillas_dirty = (
        ...
        circ.append(
            mcg_up_to_diag,
            [target_qubit] + control_qubits + q_ancillas_zero + ancillas_dirty,
        )
...
def _reverse_qubit_state(state, basis_state):
    ...
    if basis_state == 0:
        m = (
            np.array([[np.conj(state[0]), np.conj(state[1])], [-state[1], state[0]]])
            / r
        )
    else:
        m = (
            np.array([[-state[1], state[0]], [np.conj(state[0]), np.conj(state[1])]])
            / r
        )
    return m
...
def _apply_ucg(m, k, single_qubit_gates):
    ...
    for j in range(2 ** (num_qubits - 1)):
        ...
        for l in range(num_col):
            m[np.array([i, i + spacing]), np.array([l, l])] = np.ndarray.flatten(
                single_qubit_gates[gate_index].dot(
                    np.array([[m[i, l]], [m[i + spacing, l]]])
                )
            ).tolist()
    return m
...
def _apply_multi_controlled_gate(m, control_labels, target_label, gate):
    ...
    for state_free in basis_states_free:
        ...
        for l in range(num_cols):
            m[np.array([e1, e2]), np.array([l, l])] = np.ndarray.flatten(
                gate.dot(np.array([[m[e1, l]], [m[e2, l]]]))
            ).tolist()
    return m
...
def _merge_UCG_and_diag(single_qubit_gates, diag):
    for (i, gate) in enumerate(single_qubit_gates):
        single_qubit_gates[i] = np.array(
            [[diag[2 * i], 0.0], [0.0, diag[2 * i + 1]]]
        ).dot(gate)
