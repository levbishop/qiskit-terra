...
def square(
    times: np.ndarray, amp: complex, period: float, phase: float = 0
) -> np.ndarray:
...
def triangle(
    ...
    """
    return amp * (
        -2 * np.abs(sawtooth(times, 1, period, (phase - np.pi / 2) / 2)) + 1
    ).astype(np.complex_)
...
def _fix_gaussian_width(
    ...
    center: float,
    sigma: float,
    zeroed_width: Union[None, float] = None,
...
def gaussian_square(
    ...
    funclist = [
        functools.partial(
            ...
            center=square_start,
            sigma=sigma,
            zeroed_width=gauss_zeroed_width,
            rescale_amp=True,
        ),
        functools.partial(
            gaussian,
...
def drag(
    ...
    gauss_deriv, gauss = gaussian_deriv(
        times, amp=amp, center=center, sigma=sigma, ret_gaussian=True
    )
    if zeroed_width is not None:
        gauss, scale_factor = _fix_gaussian_width(
            gauss,
            amp=amp,
            center=center,
            sigma=sigma,
