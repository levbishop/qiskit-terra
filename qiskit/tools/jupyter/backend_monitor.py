...
class BackendMonitor(Magics):
    ...
    def qiskit_backend_monitor(self, line="", cell=None):
        ...
        if not backend.configuration().simulator:
            tab_contents.extend(
                ["Qubit Properties", "Multi-Qubit Gates", "Error Map", "Job History"]
            )
            details.extend(
                [
                    qubits_tab(backend),
                    gates_tab(backend),
                    detailed_map(backend),
                    job_history(backend),
                ]
            )
        ...
        title_widget = widgets.HTML(
            value=title_html, layout=widgets.Layout(margin="0px 0px 0px 0px")
        )
...
def _load_jobs_data(self, change):
    ...
    if change["new"] == 4 and not self._did_jobs:
        ...
        year = widgets.Output(
            layout=widgets.Layout(
                display="flex-inline", align_items="center", min_height="400px"
            )
        )
...
def _backend_monitor(backend):
    ...
    if not backend.configuration().simulator:
        ...
        details.extend(
            [qubits_tab(backend), gates_tab(backend), detailed_map(backend), jobs]
        )
...
def qubits_tab(backend):
    ...
    header_html = header_html.format(
        key="last_update_date", value=props["last_update_date"]
    )
...
def gates_tab(backend):
    ...
    update_date_widget = widgets.HTML(
        value=header_html, layout=widgets.Layout(grid_area="top")
    )
...
tr:nth-child(even) {background-color: #f6f6f6;};
    ...
    left_table_widget = widgets.HTML(
        value=left_table, layout=widgets.Layout(grid_area="left")
    )
    middle_table_widget = widgets.HTML(
        value=middle_table, layout=widgets.Layout(grid_area="middle")
    )
    right_table_widget = widgets.HTML(
        value=right_table, layout=widgets.Layout(grid_area="right")
    )
...
def detailed_map(backend):
    ...
    single_widget = widgets.Output(
        layout=widgets.Layout(
            display="flex-inline", grid_area="left", align_items="center"
        )
    )

    cmap_widget = widgets.Output(
        layout=widgets.Layout(
            display="flex-inline",
            grid_area="top",
            width="auto",
    ...
    with single_widget:
        ...
        single_cb = mpl.colorbar.ColorbarBase(
            ax1, cmap=cm.viridis, norm=single_norm, orientation="horizontal"
        )
    ...
    with cx_widget:
        ...
        cx_cb = mpl.colorbar.ColorbarBase(
            ax2, cmap=cm.viridis, norm=cx_norm, orientation="horizontal"
        )
    ...
    out_box = widgets.GridBox(
        [single_widget, cmap_widget, cx_widget],
        layout=widgets.Layout(
            grid_template_rows="auto auto",
            grid_template_columns="33% 33% 33%",
            grid_template_areas="""
...
def plot_job_history(jobs, interval="year"):
    ...
    if interval == "year":
        bins = [
            (current_time - datetime.timedelta(days=k * 365 / 12)) for k in range(12)
        ]
    elif interval == "month":
    ...
    if interval == "year":
        labels = [
            "{}-{}".format(str(bins[b].year)[2:], MONTH_NAMES[bins[b].month])
            for b in nz_idx
        ]
    else:
        labels = [
            "{}-{}".format(MONTH_NAMES[bins[b].month], bins[b].day) for b in nz_idx
        ]
