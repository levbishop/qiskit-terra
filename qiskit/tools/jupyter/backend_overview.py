...
def backend_widget(backend):
    ...
    is_oper = widgets.HTML(
        value="<h5></h5>", layout=widgets.Layout(justify_content="center")
    )

    least_busy = widgets.HTML(
        value="<h5></h5>", layout=widgets.Layout(justify_content="center")
    )
    ...
    t12_widget = widgets.HTML(
        value="<h5>{t1} / {t2} {units}</h5>".format(
    ...
    out = widgets.VBox(
        ...
        layout=widgets.Layout(
            display="inline-flex", flex_flow="column", align_items="center"
        ),
    )
...
def generate_jobs_pending_widget():
    ...
    jobs_widget = widgets.HBox(
        ...
        layout=widgets.Layout(
            max_width="250px", min_width="250px", justify_content="center"
        ),
    )
