    ...
    def qiskit_job_status(self, line="", cell=None):
        ...
        for var in line_vars:
            ...
            if "#" not in var:
                ...
                if iter_var:
                    ...
                    if isinstance(
                        self.shell.user_ns[var], qiskit.providers.basejob.BaseJob
                    ):
        ...
        for idx, job_var in enumerate(jobs):
            ...
            if multi_job:
                ...
                idx_str = ""
            header = "<p style='{style}'>Job Status {id}: %s </p>".format(
                id=idx_str, style=style
            )
            status = widgets.HTML(value=header % job_var.status().value)

            thread = threading.Thread(
                target=_html_checker,
                args=(job_var, args.interval, status, header, _interval_set),
            )
            thread.start()
