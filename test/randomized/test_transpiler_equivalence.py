...
class QCircuitMachine(RuleBasedStateMachine):
    ...
    @rule(
        gate=st.sampled_from(variadic_gates),
    ...
    def add_c_if_last_gate(self, carg, data):
        ...
        assume(
            not isinstance(last_gate[0], Measure) or creg != last_gate[2][0].register
        )
    ...
    def equivalent_transpile(self, backend, opt_level):
        ...
        assume(
            backend is None or backend.configuration().n_qubits >= len(self.qc.qubits)
        )
        ...
        aer_counts = (
            execute(self.qc, backend=self.backend, shots=shots).result().get_counts()
        )

        try:
            xpiled_qc = transpile(
                self.qc, backend=backend, optimization_level=opt_level
            )
        ...
        count_differences = dicts_almost_equal(
            aer_counts, xpiled_aer_counts, 0.05 * shots
        )
