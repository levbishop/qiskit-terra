...
class SchedulerTestCase(QiskitTestCase):
    ...
    def assertSchedulerRaises(self, circuit, passmanager, expected, exception_type):
        ...
        with self.assertLogs(logger, level="INFO") as cm:
            self.assertRaises(
                exception_type, transpile, circuit, pass_manager=passmanager
            )
        self.assertEqual([record.message for record in cm.records], expected)
...
class TestUseCases(SchedulerTestCase):
    ...
    def test_non_idempotent_pass(self):
        ...
        self.passmanager.append(
            [PassF_reduce_dag_property(), PassF_reduce_dag_property()]
        )
        self.passmanager.append(PassF_reduce_dag_property())
        self.assertScheduler(
            self.circuit,
    ...
    def test_fenced_dag(self):
        ...
        self.assertSchedulerRaises(
            circ,
            self.passmanager,
            ["run analysis pass PassI_Bad_AP", "cx_runs: {(5, 6, 7, 8)}"],
            TranspilerAccessError,
        )
    ...
    def test_analysis_pass_is_idempotent(self):
        ...
        self.assertScheduler(
            self.circuit,
            passmanager,
            ["run analysis pass PassE_AP_NR_NP", "set property as 1"],
        )
...
class TestControlFlowPlugin(SchedulerTestCase):
    ...
    def test_callable_control_flow_plugin(self):
        ...
        self.assertEqual(
            controllers_length - 1, len(FlowController.registered_controllers)
        )
...
class TestLogPasses(QiskitTestCase):
    ...
    def assertPassLog(self, passmanager, list_of_passes):
        ...
        current_names = [
            pass_log["name"] for pass_log in passmanager.property_set["pass_raw_log"]
        ]
        self.assertEqual(current_names, list_of_passes)

        # All the passes run under the second
        current_running_time = [
            pass_log["running_time"] < 1
            for pass_log in passmanager.property_set["pass_raw_log"]
        ]
        ...
        formatted_log = [
            "%s: %.5f (ms)"
            % (pass_raw_log["name"], (pass_raw_log["running_time"]) * 1000)
            for pass_raw_log in passmanager.property_set["pass_raw_log"]
        ]
    ...
    def test_passes(self):
        ...
        self.assertPassLog(
            passmanager, ["PassA_TP_NR_NP", "PassC_TP_RA_PA", "PassB_TP_RA_PA"]
        )
