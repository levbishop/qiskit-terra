...
class TestQI(QiskitTestCase):
    ...
    def test_vectorize(self):
        ...
        paul = [5, 5, -1j, -3]
        test_pass = (
            np.linalg.norm(vectorize(mat) - col) == 0
            and np.linalg.norm(vectorize(mat, method="col") - col) == 0
            and np.linalg.norm(vectorize(mat, method="row") - row) == 0
            and np.linalg.norm(vectorize(mat, method="pauli") - paul) == 0
        )
    ...
    def test_devectorize(self):
        ...
        paul = [5, 5, -1j, -3]
        test_pass = (
            np.linalg.norm(devectorize(col) - mat) == 0
            and np.linalg.norm(devectorize(col, method="col") - mat) == 0
            and np.linalg.norm(devectorize(row, method="row") - mat) == 0
            and np.linalg.norm(devectorize(paul, method="pauli") - mat) == 0
        )
        self.assertTrue(test_pass)
    ...
    def test_outer(self):
        ...
        op_yz = [[1, 0], [1j, 0]]
        test_pass = (
            np.linalg.norm(outer(v_z) - rho_z) == 0
            and np.linalg.norm(outer(v_y) - rho_y) == 0
            and np.linalg.norm(outer(v_y, v_z) - op_yz) == 0
            and np.linalg.norm(outer(v_z, v_y) - op_zy) == 0
        )
        self.assertTrue(test_pass)
    ...
    def test_concurrence(self):
        ...
        test_pass = (
            concurrence(psi1) == 0.0
            and concurrence(rho1) == 1.0
            and concurrence(rho2) == 1.0
            and concurrence(rho3) == 0.0
    ...
    def test_chop_imaginary(self):
        ...
        for i, expected in enumerate(
            [0.0 + 0.0j, 1.0456789, 4 + 0.0j, 0 + 3j, 0.0, 2 + 6j]
        ):
            self.assertEqual(expected, res[i])
    ...
    def test_mutual_information(self):
        input_state = np.array(
            [[0.5, 0.25, 0.75, 1], [1, 0, 1, 0], [0.5, 0.5, 0.5, 0.5], [0, 1, 0, 1]]
        )
    ...
    def test_choi_to_rauli(self):
        ...
        expected = np.array(
            [
                [2.0 + 0.0j, 2.25 + 0.0j, 0.0 + 0.75j, -1.0 + 0.0j],
                [1.75 + 0.0j, 2.5 + 0.0j, 0.0 - 1.5j, 0.75 + 0.0j],
                [0.0 - 0.25j, 0.0 + 0.5j, -0.5 + 0.0j, 0.0 - 1.25j],
                [0.0 + 0.0j, 0.25 + 0.0j, 0.0 - 1.25j, 1.0 + 0.0j],
            ]
        )
    ...
    def test_random_density_matrix_invalid_method(self):
        self.assertRaises(QiskitError, random_density_matrix, 2, method="Special")
