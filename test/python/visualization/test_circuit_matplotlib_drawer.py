...
class TestMatplotlibDrawer(QiskitVisualizationTestCase):
    ...
    def test_plot_barriers(self):
        ...
        filename = self._get_resource_path(
            "visualization/references/current_matplotlib_ref.png"
        )
        visualization.circuit_drawer(
            qc, output="mpl", plot_barriers=True, filename=filename
        )
        self.addCleanup(os.remove, filename)
        ...
        visualization.circuit_drawer(
            qc, output="mpl", plot_barriers=False, filename=filename
        )
        self.addCleanup(os.remove, filename)
        ...
        no_barriers_filename = self._get_resource_path(
            "current_no_barriers_matplotlib_ref.png"
        )
        visualization.circuit_drawer(
            qc1, output="mpl", justify="None", filename=no_barriers_filename
        )
