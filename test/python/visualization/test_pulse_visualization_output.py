...
class TestPulseVisualizationImplementation(QiskitVisualizationTestCase):
    ...
    instr_matplotlib_reference = path_to_diagram_reference(
        "instruction_matplotlib_ref.png"
    )
    schedule_matplotlib_reference = path_to_diagram_reference(
        "schedule_matplotlib_ref.png"
    )
    ...
    def sample_schedule(self):
        ...
        sched = sched.insert(
            0, PersistentValue(value=0.2 + 0.4j)(self.device.q[0].controls[0])
        )
        ...
        sched = sched.insert(
            90, acquire(self.device.q[1], self.device.mem[1], self.device.c[1])
        )
        sched = sched + sched
