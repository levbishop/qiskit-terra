...
class TestInstructionToQobjConverter(QiskitTestCase):
    ...
    def setUp(self):
        self.device = DeviceSpecification(
            qubits=[Qubit(0, DriveChannel(0), MeasureChannel(0), AcquireChannel(0))],
    ...
    def test_snapshot(self):
        ...
        valid_qobj = PulseQobjInstruction(
            name="snapshot", t0=0, label="label", type="type"
        )
...
class TestQobjToInstructionConverter(QiskitTestCase):
    ...
    def setUp(self):
        ...
        self.pulse_library = [
            PulseLibraryItem(
                name=self.linear.name, samples=self.linear.samples.tolist()
            )
        ]
    ...
    def test_acquire(self):
        ...
        cmd = Acquire(
            10,
            Discriminator(name="test_disc", params={"test_params": 1.0}),
        ...
        qobj = PulseQobjInstruction(
            ...
            register_slot=[0, 1],
            kernels=[
                QobjMeasurementOption(name="test_kern", params={"test_params": "test"})
            ],
            discriminators=[
    ...
    def test_parameterized_frame_change(self):
        ...
        evaluated_instruction = converted_instruction.bind_parameters(2.0)
    ...
    def test_parameterized_persistent_value(self):
        """Test converted qobj from PersistentValueInstruction."""
        cmd = PersistentValue(value=0.5 + 0.0j)
        ...
        evaluated_instruction = converted_instruction.bind_parameters(P1=0.5, P2=0.0)
...
class TestLoConverter(QiskitTestCase):
    ...
    def test_qubit_los(self):
        ...
        converter = LoConfigConverter(
            PulseQobjExperimentConfig, [1.2], [3.4], [(0.0, 5.0)], [(0.0, 5.0)]
        )
