...
class TestContinuousPulses(QiskitTestCase):
    ...
    def test_square(self):
        ...
        # with new phase
        square_arr_phased = continuous.square(
            times, amp=amp, period=period, phase=np.pi / 2
        )
    ...
    def test_sawtooth(self):
        ...
        # with new phase
        sawtooth_arr_phased = continuous.sawtooth(
            times, amp=amp, period=period, phase=np.pi / 2
        )
        ...
        self.assertAlmostEqual(
            (sawtooth_arr[1] - sawtooth_arr[0]) / dt, 2 * amp / period
        )
    ...
    def test_triangle(self):
        ...
        # with new phase
        triangle_arr_phased = continuous.triangle(
            times, amp=amp, period=period, phase=np.pi / 2
        )
        ...
        # test slope
        self.assertAlmostEqual(
            (triangle_arr[1] - triangle_arr[0]) / dt, 4 * amp / period
        )
    ...
    def test_gaussian_square(self):
        ...
        gaussian_square_arr = continuous.gaussian_square(
            times, amp, center, width, sigma
        )
        ...
        rise_times, dt_rise = np.linspace(
            gauss_rise_end_time - epsilon,
            gauss_rise_end_time + epsilon,
            1001,
            retstep=True,
        )
        fall_times, dt_fall = np.linspace(
            gauss_fall_start_time - epsilon,
            gauss_fall_start_time + epsilon,
            1001,
            retstep=True,
        )
        gaussian_square_rise_arr = continuous.gaussian_square(
            rise_times, amp, center, width, sigma
        )
        gaussian_square_fall_arr = continuous.gaussian_square(
            fall_times, amp, center, width, sigma
        )
