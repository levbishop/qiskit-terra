...
class TestJupyter(QiskitTestCase):
    ...
    def _execute_notebook(self, filename, qe_token=None, qe_url=None):
        ...
        execute_preprocessor = ExecutePreprocessor(
            timeout=TIMEOUT, kernel_name=JUPYTER_KERNEL
        )
        ...
        if qe_token and qe_url:
            ...
            top_str += "IBMQ.enable_account('{token}', '{url}')".format(
    ...
    def test_jupyter_jobs_pbars(self):
        ...
        self._execute_notebook(
            self._get_resource_path("notebooks/test_pbar_status.ipynb")
        )
    ...
    def test_backend_tools(self, qe_token, qe_url):
        ...
        self._execute_notebook(
            self._get_resource_path("notebooks/test_backend_tools.ipynb"),
            qe_token=qe_token,
            qe_url=qe_url,
        )
