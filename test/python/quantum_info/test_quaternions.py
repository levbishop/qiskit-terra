...
class TestQuaternions(QiskitTestCase):
    ...
    def test_orthogonality(self):
        ...
        self.assertTrue(
            np.allclose(self.mat2.dot(self.mat2.T), np.identity(3, dtype=float))
        )
    ...
    def test_rotation(self):
        ...
        self.assertTrue(
            np.allclose(neg_quat.to_matrix(), self.quat_unnormalized.to_matrix())
        )
    ...
    def test_two_euler_angle_0123456789(self):
        """Quaternion should return a correct sequence of zyz representation
           ...
           angle = 0.123456789 """
        rand_rot_angle = 0.123456789
        some_quat = quaternion_from_axis_rotation(
            rand_rot_angle, "z"
        ) * quaternion_from_axis_rotation(np.pi, "y")
        self.assertTrue(
            np.allclose(some_quat.to_zyz(), np.array([rand_rot_angle, np.pi, 0]))
        )
    ...
    def test_quaternion_from_rotation_invalid_axis(self):
        ...
        self.assertRaises(
            ValueError, quaternion_from_axis_rotation, rand_angle, rand_axis
        )
