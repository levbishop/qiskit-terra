...
class TestStates(QiskitTestCase):
    ...
    def test_state_fidelity(self):
        ...
        self.assertAlmostEqual(
            state_fidelity(psi1, psi1), 1.0, places=7, msg="vector-vector input"
        )
        self.assertAlmostEqual(
            state_fidelity(psi1, psi2), 0.0, places=7, msg="vector-vector input"
        )
        self.assertAlmostEqual(
            state_fidelity(psi1, rho1), 1.0, places=7, msg="vector-matrix input"
        )
        self.assertAlmostEqual(
            state_fidelity(psi1, mix), 0.25, places=7, msg="vector-matrix input"
        )
        self.assertAlmostEqual(
            state_fidelity(psi2, rho1), 0.0, places=7, msg="vector-matrix input"
        )
        self.assertAlmostEqual(
            state_fidelity(psi2, mix), 0.25, places=7, msg="vector-matrix input"
        )
        self.assertAlmostEqual(
            state_fidelity(rho1, psi1), 1.0, places=7, msg="matrix-vector input"
        )
        self.assertAlmostEqual(
            state_fidelity(rho1, rho1), 1.0, places=7, msg="matrix-matrix input"
        )
        self.assertAlmostEqual(
            state_fidelity(mix, mix), 1.0, places=7, msg="matrix-matrix input"
        )

    def test_state_fidelity_qubit(self):
        state0 = np.array([1.0 + 0.0j, 0.0 + 0.0j])
        state1 = np.array([0.0 + 0.0j, 1.0 + 0.0j])
    ...
    def test_projector(self):
        state0 = np.array([1.0 + 0.0j, 0.0 + 0.0j])
        state1 = projector(np.array([0.0 + 0.0j, 1.0 + 0.0j]))
        self.assertEqual(state_fidelity(state0, state0), 1.0)
        self.assertEqual(state_fidelity(state1, state1), 1.0)
        self.assertEqual(state_fidelity(state0, state1), 0.0)
    ...
    def test_random_state(self):
        ...
        for ii in range(number):
            ...
            E_P0 = (E_P0_last * ii) / (ii + 1) + state_fidelity(
                state, random_state(2 ** 3, seed=ii)
            ) / (ii + 1)
            E_P0_last = E_P0
    ...
    def test_purity_pure_state(self):
        ...
        state_2 = (1 / np.sqrt(3)) * (
            basis_state("00", 2) + basis_state("01", 2) + basis_state("11", 2)
        )
        state_3 = 0.5 * (
            basis_state("000", 3)
            + basis_state("001", 3)
    ...
    def test_purity_mixed_state(self):
        state_1 = 0.5 * (
            projector(basis_state("0", 1)) + projector(basis_state("1", 1))
        )
        state_2 = (1 / 3.0) * (
            projector(basis_state("00", 2))
