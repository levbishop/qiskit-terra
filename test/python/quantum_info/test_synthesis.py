...
def make_oneq_cliffords():
    ...
    oneq_cliffords = [
        Operator(ixyz @ ih @ irs)
        for ixyz in ixyz_list
        for ih in ih_list
        for irs in irs_list
    ]
    ...
    def check_one_qubit_euler_angles(self, operator, tolerance=1e-14):
        ...
        with self.subTest(operator=operator):
            ...
            if maxdist > 0.1:
                maxdist = np.max(np.abs(target_unitary + decomp_unitary))
            self.assertTrue(
                np.abs(maxdist) < tolerance, "Worst distance {}".format(maxdist)
            )
...
class TestTwoQubitWeylDecomposition(QiskitTestCase):
    ...
    def test_two_qubit_weyl_decomposition_a00(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
    ...
    def test_two_qubit_weyl_decomposition_aa0(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
    ...
    def test_two_qubit_weyl_decomposition_aaa(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
    ...
    def test_two_qubit_weyl_decomposition_aama(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
        """Verify Weyl KAK decomposition for U~Ud(a,a,-a)"""
        for aaa in (
    ...
    def test_two_qubit_weyl_decomposition_ab0(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
    ...
    def test_two_qubit_weyl_decomposition_abb(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
    ...
    def test_two_qubit_weyl_decomposition_abmb(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
        """Verify Weyl KAK decomposition for U~Ud(a,b,-b)"""
        for aaa in (
    ...
    def test_two_qubit_weyl_decomposition_aac(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
    ...
    def test_two_qubit_weyl_decomposition_abc(
        self, smallest=1e-18, factor=9.8, steps=11
    ):
...
class TestTwoQubitDecomposeExact(QiskitTestCase):
    ...
    def test_exact_nonsupercontrolled_decompose(self):
        ...
        with self.assertWarns(
            UserWarning, msg="Supposed to warn when basis non-supercontrolled"
        ):
